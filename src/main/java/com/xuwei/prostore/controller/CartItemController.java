...
public class CartItemController {
    ...
    public ResponseEntity<ApiResponse> addItemToCart(@RequestParam(required = false) Long cartId,
        ...
        try {
            ...
            cartItemService.addItemToCart(cartId, productId,
                    quantity);
            return ResponseEntity.ok(new ApiResponse("Add Item " +
                    "Success", null));
    ...
    public ResponseEntity<ApiResponse> removeItemFromCart(@PathVariable Long cartId,
        ...
        try {
            ...
            return ResponseEntity.ok(new ApiResponse("Remove Item " +
                    "Success", null));
        } catch (ResourceNotFoundException e) {
            return ResponseEntity.status(NOT_FOUND).body(new ApiResponse(e.getMessage(), null));
        }
    ...
    public ResponseEntity<ApiResponse> updateItemQuantity(@PathVariable Long cartId,
                                                          ...
                                                          @RequestParam Integer quantity) {
        try {
            cartItemService.updateItemQuantity(cartId, itemId,
                    quantity);
            return ResponseEntity.ok(new ApiResponse("Update Item " +
                    "Success", null));
        } catch (ResourceNotFoundException e) {
            return ResponseEntity.status(NOT_FOUND).body(new ApiResponse(e.getMessage(), null));
        }
    ...
    public ResponseEntity<ApiResponse> getCartItem(@PathVariable Long itemId) {
        try {
            CartItemDto cartItem =
                    cartItemService.getCartItem(itemId);
